...
esri.filter = function (object,thisObject,callback) {
...
esri.request = function (options,request) {
...
esri.setRequestPreCallback = function () {
...
esri.urlToObject = function (url) {
    /// <summary>Converts the URL arguments to an object representation. The object format is {path: , query:{key:}}</summary>
...
esri.IdentityManagerBase.prototype = 
{
    ...
    generateToken: function (serverInfo,userInfo,options) {
        ...
        /// <param name="options" type="Object" optional="true">Optional parameters. (As of 3.0). isAdmin Indicate that the token should be generated using the token service deployed with the ArcGIS Server Admin API. The default value is false. </param>
        /// <return type="dojo.Deferred">dojo.Deferred</return>
    },
    getCredential: function (url,options) {
        ...
        /// <param name="options" type="Object" optional="true">Optional parameters. (As of 3.0). retry Determines if the method should make additional attempts to get the credentials after a failure. token Token used for a previous unsuccessful attempt to fetch the given url error Error object returned by the server from a previous attempt to fetch the given url. </param>
        /// <return type="dojo.Deferred ">dojo.Deferred </return>
    },
    ...
    registerToken: function (properties) {
        ...
        /// <param name="properties" type="Object" optional="false">An object with the following properties: serverThis is the root URL for the ArcGIS Online REST APIhttp://www.arcgis.com/sharing/rest tokenThe access token. userIdThe id for the user who owns the access token. expiresToken expiration time specified as number of milliseconds since 1 January 1970 00:00:00 URC. sslSet this to true if the user has an ArcGIS Online Organizational Account and the organization is configured to allow access to resources only through SSL. </param>
    },
    ...
    setRedirectionHandler: function (handler) {
        ...
        /// <param name="handler" type="Object" optional="false">An object containing the following redirection properties: resourceUrl The URL of the secure resource that triggers the redirection to the ArcGIS.com sign-in page. serverInfo ServerInfo object describing the server where the secure resource is hosted. signInPage URL of the sign-in page where users will be redirected. returnUrlParamName The application URL where the sign-in page redirects after a successful log-in. To create the return URL append the application's URL to signInPage as a parameter. returnUrlParamName contains the name of the parameter. </param>
    },
    signIn: function (url,serverInfo,options) {
        ...
        /// <param name="options" type="Object" optional="true">Optional parameters. (As of 3.0). error Error object returned by the server from a previous attempt to fetch the given url. isAdmin Indicate that the token should be generated using the token service deployed with the ArcGIS Server Admin API. The default value is false. token Token used for previous unsuccessful attempts to fetch the given url </param>
        /// <return type="dojo.Deferred ">dojo.Deferred </return>
    },
    toJson: function () {
...
esri.InfoTemplate = function (title,content) {
    ...
    /// <field name="content" type="String |Function">The template for defining how to format the content used in an InfoWindow. Starting with version 2.2 the content can be defined using either a string or a function. Prior to 2.2, only a string can be specified. Starting with version 2.2 the content for an InfoWindow can be defined using either a string or a function. This provides the developer the ability to easily format and customize the InfoWindow contents. Prior to 2.2, only a string can be specified. View the Format Info Window Content help topic for more details.</field>
    /// <field name="title" type="String|Function">The template for defining how to format the title used in an InfoWindow.You can format the title by specifying either a string value or a function. See the content property for more details. In most cases, the title is specified as either a simple string or a function that returns a simple string.View the Format Info Window Content help topic for more details.</field>
};
...
esri.Map = function (divId,options) {
    ...
    /// <field name="extent" type="Extent">The current extent of the map in map units. See Setting and using extents in a map for more information about extents.</field>
    ...
    /// <field name="height" type="Number">Current height of the map in screen pixels. This property is read-only. The height and width of the map are normally set in the HTML DIV or other HTML element containing the map container.</field>
    ...
    /// <field name="isClickRecenter" type="Boolean">When true, the key sequence of shift then click to recenter the map is enabled. See Map navigation for more details.</field>
    /// <field name="isDoubleClickZoom" type="Boolean">When true, double click zoom is enabled. This allows a user to zoom and recenter the map by double clicking the mouse. See Map navigation for more details.</field>
    /// <field name="isKeyboardNavigation" type="Boolean">When true, keyboard navigation is enabled. This allows users to pan the keyboard using the arrow keys and to zoom using the + and - keys. See Map navigation for more details.</field>
    /// <field name="isPan" type="Boolean">When true, map panning is enabled using the mouse. See Map navigation for more details.</field>
    /// <field name="isPanArrows" type="Boolean">When true, pan arrows are displayed around the edge of the map. See Map navigation for more details.</field>
    /// <field name="isRubberBandZoom" type="Boolean">When true, rubberband zoom is enabled. This allows users to draw a bounding box zoom area using the mouse. See Map navigation for more details.</field>
    /// <field name="isScrollWheelZoom" type="Boolean">When true, the mouse scroll wheel zoom is enabled. See Map navigation for more details.</field>
    /// <field name="isShiftDoubleClickZoom" type="Boolean">When true, shift double click zoom is enabled. This allows a user to zoom and recenter the map by shift + double clicking the mouse. See Map navigation for more details.</field>
    /// <field name="isZoomSlider" type="Boolean">When true, the zoom slider is displayed on the map. See Map navigation for more details.</field>
    ...
    /// <field name="width" type="Number">Current width of the map in screen pixels. This property is read-only. The height and width of the map are normally set in the HTML DIV or other HTML element containing the map container.</field>
};
...
esri.Map.prototype = 
{
    ...
    enableSnapping: function (snapOptions) {
        ...
        /// <param name="snapOptions" type="Object" optional="true"> tolerance Specify the radius of the snapping circle in pixels. The default value is 15 pixels. layerInfos An array of layerInfo objects that define the snapping target layers. All values are optional. If no snapping options are set the default values will be used. layer Reference to a feature or graphics layer that will be a target snapping layer. The default option is to set all feature and graphics layers in the map to be target snapping layers. snapToPoint Default is true. When true snapping to points will be enabled for layers with point geometry. snapToVertex Default is true. When true snapping to vertices will be enabled for layers with polyline or polygon geometry. snapToEdge Default is true. When true snapping to edges will be enabled for layers with polyline or polygon geometry. <SimpleMarkerSymbol> snapPointSymbol Define a symbol for the snapping location. The default symbol is a simple marker symbol with the following properties: size:15px,color:cyan,style:STYLE_CROSS. alwaysSnap When true, snapping is always enabled. When false users press the snapKey to enable snapping. The default value is false. <dojo.key> snapKey When alwaysSnap is set to false use this option to define the key users press to enable snapping. The default values is the CTRL key. </param>
    ...
    reorderLayer: function (layer,index) {
        ...
        /// <param name="layer" type="Layer" optional="false">The layer to be moved. (As of v1.4) Beginning with version 1.4, use this parameter in place of "layerId". In versions prior to v1.4, use "layerID" in place of "layer". Type: Definition: The ID of the layer to be moved. This ID is assigned in Layer.id.</param>
...
esri.SpatialReference = function (json) {
    ...
    /// <param name="json" type="Object" optional="false">The REST JSON representation of the spatial reference: {"wkid" : }</param>
    ...
    /// <field name="wkt" type="String">The well-known text that defines a spatial reference. Many browser have a limit to the length of a GET request of approximately 2048 characters. When using well-known text to specify the spatial reference you can easily exceed this limit. In these cases, you will need to setup and use a proxy page.</field>
};
...
esri.arcgis.utils.createMap = function (itemId_or_itemInfo,mapDiv,options) {
    ...
    /// <param name="options" type="Object" optional="true">Optional parameters that define the map functionality: mapOptions See the optional parameters for the esri.map constructor for more details. bingMapsKey The Bing Maps Key, required if working with Microsoft Bing Maps. ignorePopups When true, webmap defined popups will not display. geometryServiceURL URL to the ArcGIS Server REST resource that represents a geometry service. For more information on constructing a URL, see The Services Directory and the REST API. </param>
    /// <return type="dojo.Deferred">dojo.Deferred</return>
};
...
esri.dijit.Directions = function (params,srcNodeRef) {
    ...
    /// <field name="geocoderResults" type="Object[]">An array of objects that defines the potential matches for the input locations. For example, if 'Flagstaff' is input as a stop there is a Flagstaff in Arizona but also one in Jamaica, South Africa and New Zealand this property provides access to the additional address possibilities. { value: "Flagstaff", //this is the value that was used by the Directions widget results:[{ extent: Extent, feature: Graphic, name: "Flagstaff, Arizona, United States" },{ extent: Extent, feature: Graphic, name: "Flagstaff, Saint James, Jamaica" },{ extent: Extent, feature: Graphic, name: "Flagstaff, Otago, New Zealand" }}</field>
...
esri.dijit.Directions.prototype = 
{
    ...
    updateStop: function (index,stop) {
...
esri.dijit.Geocoder.prototype = 
{
    ...
    select: function (result) {
        ...
        /// <param name="result" type="Object" optional="false">An object with the following properties:{ extent: , feature: , name: }</param>
    },
    show: function () {
...
esri.dijit.InfoWindow = function () {
    /// <summary>An InfoWindow is an HTML popup. It often contains the attributes of a Graphic. The default behavior on a Graphic is to show the InfoWindow after a click on the Graphic. An InfoTemplate is required for this default behavior.In addition, the InfoWindow can be used to display custom content on the map. Note: Only one info window is displayed on the map at a time.</summary>
...
esri.dijit.Popup.prototype = 
{
    ...
    show: function (location,options) {
        ...
        /// <param name="options" type="Object" optional="true">Optional parameters. (As of 3.0). closestFirst When using the setFeatures method to display multiple features in the popup, enable this option to display the feature closest to the specified location first. </param>
    },
};
...
esri.dijit.editing = function (params,srcNodeRef) {
    /// <summary>Creates a new TimeSlider object.</summary>
    /// <param name="params" type="String" optional="false">Parameters for the time slider object. See the options table below for parameters.&#10;
    /// &#60;Boolean&#62; excludeDataAtLeadingThumb&#10;
    /// &#60;Boolean&#62; excludeDataAtTrailingThumb</param>
...
esri.layers.ArcGISDynamicMapServiceLayer = function (url,options) {
    ...
    /// <field name="suspended" type="Boolean">When true the layer is suspended. A layer is considered to be suspended when one of the following is true:The layer is hidden.The layer is not visible at the current map scale.The layer is explicitly suspended by calling the Layer.suspend method.</field>
    ...
    /// <field name="useMapImage" type="Boolean">When true, the image is saved to the server, and a JSON formatted response is sent to the client with the URL location of the image. A second call is automatically made to the server to retrieve the image. This value is set in the constructor using useMapImage. Deprecated at v2.0. Prior to v2.0 this option was used when some export map requests exceeded the limit of 2000 characters and it configured the map to use the proxy all the time. This is not the optimal behavior so at v2.0 the switch to use the proxy will automatically happen for long requests.</field>
...
esri.layers.CodedValueDomain = function () {
    ...
    /// <field name="codedValues" type="Object[]">An array of the coded values in the domain. Each object consists of two properties: name and code.</field>
};
...
{
    getImageUrl: function (callback,extent,width,height) {
...
esri.layers.FeatureLayer = function (url,options) {
    ...
    /// <field name="defaultDefinitionExpression" type="String">Metadata describing the default definition expression for the layer as defined by the service. The default definition expression limits the features available for display and query. You cannot override this value but you define additional filters on the default expression using the setDefinitionExpression method. For example, if the default definition expression is set to display data where "STATE_NAME = 'California'" you could use setDefinitionExpression to only display a subset of the features in California e.g., "COUNTY='San Diego'".</field>
    ...
    /// <field name="editFieldsInfo" type="Object">Indicates the field names for the editor fields. If ownership based access control is not enabled for the feature service the value will be null. The object has the following properties: { creatorField, creationDateField, editorField, editDateField }Requires ArcGIS Server feature service version 10.1 or greater.</field>
    ...
    /// <field name="hasAttachments" type="Boolean">True if attachments are enabled on the feature layer. Use the queryAttachmentInfos method to determine if the feature has attachments. If the layer is editable (isEditable) and supports attachments the following operations can be performed.Add attachments to features.Delete existing attachments.</field>
    ...
    /// <field name="ownershipBasedAccessControlForFeatures" type="Object"> Indicates the ownership access control configuration. If ownership based access control is not enabled for the feature service the value will be null. If ownership based access control is not enabled for the feature service the value will be null. This object has two properties allowDeleteToOthers and allowUpdateToOthers. When true features can be deleted or updated by users other than the creator. { allowUpdateToOthers, allowDeleteToOthers}</field>
    /// <field name="relationships" type="Object[]"> Each element in the array is an object that describes the layer's relationship with another layer or table. Use the queryRelatedFeatures method to retrieve features or records related to a feature in this layer. The object has the following properties: id Unique ID for the relationship name Name of the relationship. relatedTableId Unique ID of the related table or layer </field>
    ...
    /// <field name="supportsAdvancedQueries" type="Boolean">When true, the layer supports orderByFields in a query operation. Requires ArcGIS Server service version 10.1 or greater</field>
    /// <field name="supportsStatistics" type="Boolean">When true, the layer supports statistical functions in query operations. Requires ArcGIS Server service version 10.1 or greater</field>
    /// <field name="suspended" type="Boolean">When true the layer is suspended. A layer is considered to be suspended when one of the following is true:The layer is hidden.The layer is not visible at the current map scale.The layer is explicitly suspended by calling the Layer.suspend method.</field>
...
esri.layers.FeatureLayer.prototype = 
{
    ...
    getEditCapabilities: function (options) {
        ...
        /// <param name="options" type="Object" optional="true">If the layer supports ownership based access control, use the options to determine if the specified user can edit features. Ownership based access control requires ArcGIS Server services version 10.1 or greater. feature Check to see if the specified feature can be edited by the specified user. userId The name of the currently logged in user. If the application uses the IdentityManager the IdentityManager supplies the layer with the userId so this parameter is not required. </param>
        /// <return type="Object">Object</return>
    },
    getEditInfo: function (feature,options) {
        ...
        /// <param name="options" type="Object" optional="true">The options object may have the following properties: action By default, the method returns a summary of the most recent edit performed on the feature. Use this option to override this behavior. 'creation' indicates creation summary is desired. 'edit' indicates edit summary is desired. </param>
    ...
    getEditSummary: function (feature,options) {
        ...
        /// <param name="options" type="Object" optional="true">The options object may have the following properties: action By default, the method returns a summary of the most recent edit performed on the feature. Use this option to override this behavior. 'creation' indicates creation summary is desired. 'edit' indicates edit summary is desired. callback If you want to customize or override certain components of the summary, provide a callback function using this option. For example: featureLayer.getEditSummary(feature, { callback: function(feature, info) { if (info && info.userId) { info.userId = "" + info.userId + ""; } return info; }}); </param>
        /// <return type="String">String</return>
    },
    ...
    setScaleRange: function (minScale,maxScale,maxScale) {
        /// <summary>Set the scale range for the layer. If minScale and maxScale are set to 0 then the layer will be visible at all scales.</summary>
        /// <param name="minScale" type="Number" optional="false">The minimum scale at which the layer is visible.</param>
    ...
    setScaleRange: function (minScale) {
        /// <summary>Set the scale range for the layer. If minScale and maxScale are set to 0 then the layer will be visible at all scales.</summary>
...
esri.layers.FeatureType = function () {
    ...
    /// <field name="domains" type="Object">Map of field names to domains. For example:{ "" : , "" : }In the above dictionary, if the domain defined for a field is an instance of InheritedDomain the application will use the domains defined for the field by the layer. If the dictionary does not include an entry for a field, the field does not have an associated domain.</field>
...
esri.layers.KMLFolder = function () {
    ...
    /// <field name="featureInfos" type="Object[]">An array of objects that describe top-level KML features ids and their types. Objects in the array have the following properties: { "type":, "id": }The type can be one of the following:Folder, GroundOverlay, Line, NetworkLink,Point, Polygon, ScreenOverlay.</field>
...
esri.layers.KMLLayer = function (id,url,options) {
    ...
    /// <field name="linkInfo" type="Object">A link info object with properties that describe the network link. The object has the following properties:{ "id": , "name": , "description": , "snippet": , "visibility": "refreshMode": , "refreshInterval": , "viewRefreshMode": , "viewRefreshTime": , "viewBoundScale": , "viewFormat": , "httpQuery": }</field>
};

esri.layers.KMLLayer.prototype = 
{
    getFeature: function (featureInfo) {
        /// <summary>Get the KML feature identified by the input feature info. The table below lists the type of objects returned for the KML feature types.KML Class name of returned objectPlacemarkGraphicGroundOverlayKMLGroundOverlayScreenOverlayKMLScreenOverlay (Not Implemented)NetworkLinkKMLLayerFolderKMLFolder</summary>
...
esri.layers.LayerDataSource = function (json) {
    ...
    /// <field name="dataSource" type="Object">The data source used to create a dynamic data layer on the fly. The data source can be one of the following:TableDataSourceQueryDataSourceJoinDataSourceRasterDataSource</field>
};
...
esri.layers.LayerTimeOptions = function () {
    ...
    /// <field name="timeDataCumulative" type="Boolean">If true, the layer will draw all features from the beginning of the data's time extent. For example, when displaying the location of a wild animal, this property will cause the layer to display the historical locations for the animal, providing a good view of the animals territory.</field>
...
esri.layers.MosaicRule = function () {
    ...
    /// <field name="ascending" type="Boolean">Indicates whether the sort should be ascending or not. This property applies to all mosaic methods where an ordering is defined except METHOD_SEAMLINE.</field>
    ...
    /// <field name="sortField" type="String">The name of the attribute field that is used together with a constant sortValue to define the mosaicking order when the mosaic method is set to METHOD_ATTRIBUTE. The ordering is defined by the absolute value of the difference between the specified sort field value and the sort base value. For example, if the sort field is Month and the sort value is 7 (July), then the ordering is defined by ABS(Month -7).</field>
...
esri.layers.QueryDataSource = function (json) {
    ...
    /// <field name="oidFields" type="String[]">An array of field names that define a unique identifier for the feature. The combiniation of one or more fields will be used to generate a unique identifier for the feature.</field>
...
esri.layers.RasterFunction = function () {
    ...
    /// <field name="arguments" type="Object">The arguments for the raster function. The structure depends on the function specified.</field>
...
{
    getTileUrl: function (row,column,level) {
...
esri.layers.TimeReference = function () {
    ...
    /// <field name="respectsDaylightSaving" type="Boolean">Indicates whether the time reference respects daylight savings time. If true, the time values are represented in both the standard time zone and the daylight savings time (when applicable). If false, time values are assumed to be in the standard time zone. </field>
...
esri.renderer.ClassBreaksRenderer = function (defaultSymbol,attributeField) {
    ...
    /// <field name="infos" type="Object[]">Each element in the array is an object that provides information about the class breaks associated with the renderer. The object has the following properties: minValue The minimum value. maxValue The maximum value. symbol The symbol used to display the value. label Label for the symbol used to draw the value. description Label for the symbol used to draw the value. </field>
};
...
esri.renderer.ClassBreaksRenderer.prototype = 
{
    addBreak: function (minValue_or_info,maxValue,symbol) {
        ...
        /// <param name="minValue_or_info" type="Number" optional="false">The value can be provided as individual arguments or as an info object with the following properties: minValue The minimum value. maxValue The maximum value. symbol The symbol used to display the value. label Label for the symbol used to draw the value. description Label for the symbol used to draw the value. </param>
...
esri.renderer.UniqueValueRenderer = function (defaultSymbol,attributeField,attributeField2,attributeField3,fieldDelimeter) {
    ...
    /// <field name="infos" type="Object[]"> Each element in the array is an object that provides information about the unique values associated with the renderer. The object has the following properties: value The unique value. symbol The symbol used to display the value. label Label for the symbol used to draw the value. description Label for the symbol used to draw the value. </field>
...
esri.renderer.UniqueValueRenderer.prototype = 
{
    addValue: function (value_or_info,symbol) {
        ...
        /// <param name="value_or_info" type="String" optional="false">Value to match with. The value can be provided as individual arguments or as an info object with the following properties: value The unique value. symbol The symbol used to display the value. label Label for the symbol used to draw the value. description Label for the symbol used to draw the value. </param>
...
esri.symbol.CartographicLineSymbol = function (style,color,width,cap,join,miterLimit) {
...
esri.symbol.Font = function (size,style,variant,weight,family) {
...
esri.symbol.SimpleFillSymbol = function (style,outline,color) {
...
esri.symbol.SimpleLineSymbol = function (style,color,width) {
...
esri.symbol.SimpleMarkerSymbol = function (style,size,outline,color) {
...
esri.symbol.TextSymbol = function (text,font,color) {
...
esri.tasks = function (text,font,color) {
    /// <summary>Creates a new TextSymbol object.</summary>
    /// <param name="text" type="String" optional="false">Text string for display in the graphics layer.</param>
    /// <param name="font" type="Font" optional="false">Font for displaying text.</param>
    /// <param name="color" type="dojo.Color" optional="false">Symbol color, which is based on dojo.Color. Colors can be denoted in the following formats:R, G, B: new dojo.Color([255,0,0]) R,G,B,A. The "A" value represents transparency where 0.0 is fully transparent and 1.0 has no transparency: new dojo.Color([255,0,0,0.25]) Hex string: new dojo.Color("#C0C0C0") Named string: new dojo.Color("blue") </param>
...
esri.tasks.AlgorithmicColorRamp = function () {
    ...
    /// <field name="algorithim" type="String">The algorithim used to generate the colors between the fromColor and toColor. Each algorithim uses different methods for generating the intervening colors. cie-lab Blends the from and to colors without traversing the intervening hue space. lab-lch The hue, saturation, value (hsv) algorithm is a linear traverse of colors between pairs: Color 1 H to Color 2 H, Color 1 S to Color 2 S, and Color 1 V to Color 2 V. hsv The lab-lch algorithm is very similar to the cie-lab but does not seek the shortest path between colors. </field>
...
esri.tasks.ClosestFacilityParameters = function () {
    ...
    /// <field name="attributeParameterValues" type="Object[]">An array of attribute parameter values that determine which network elements can be used by a vehicle. Each object has the following properties:attributeName The name of the attribute.parameterNameThe parameter name.parameterValueThe parameter value. </field>
    ...
    /// <field name="directionsLanguage" type="String">The language used when computing directions. If not specified the task will use the language defined in the network layer used by the RouteTask. The default language defined by the NAServer is en_US, the server administrator is responsible for adding additional languages.</field>
    /// <field name="directionsLengthUnits" type="String">The length units used when computing directions. If not specified the task will use the value defined by the routing network layer is used. Possible values are "esriFeet", "esriKilometers","esriMeters", "esriMiles","esriNauticalMiles" and "esriYards".</field>
    ...
    /// <field name="outputGeometryPrecision" type="Number">The output geometry precision. When 0, no generalization of the output geometry is performed. Positive values represent the MaximumAllowableOffset parameter used by generalize.</field>
    ...
    /// <field name="outputLines" type="String">The type of output lines to be generated in the result. The default is as defined in the specific routing network layer used in your RouteTask. See NAOutputLine for a list of valid values.</field>
    ...
    /// <field name="restrictionAttributes" type="String[]">The list of network attribute names to be used as restrictions with the analysis. The default is as defined in the specific routing network layer used in your RouteTask. Possible values are listed in the Service Directory under Network Dataset > Network Attributes. You can also specify a value of none to indicate that no network attributes should be used as restrictions. If you specify an empty array, it will default to the default of the service.</field>
    ...
    /// <field name="timeOfDay" type="Date">The arrival or departure date and time. For example, if the travelDirection is set to TO_FACILITY and timeOfDayUsage is set to "end" and timeOfDay is set to 8:00 a.m., the returned route(s) will be setup to arrive at the facility at 8:00 a.m. local time. Requires ArcGIS Server service version 10.1 or greater.</field>
    /// <field name="timeOfDayUsage" type="String">Defines the way the timeOfDay value is used. The default value is defined in the network layer. Requires ArcGIS Server service version 10.1 or greater.</field>
...
esri.tasks.Locator.prototype = 
{
    addressToLocations: function (params,callback,errback) {
        ...
        /// <param name="params" type="Object" optional="false">Specify the address and optionally specify the outFields and searchExtent. The searchExtent parameter requires a locator published with ArcGIS Server 10.1 or greater. address Required The address argument is data object that contains properties representing the various address fields accepted by the corresponding geocode service. These fields are listed in the addressFields property of the associated geocode service resource. For example, if the addressFields of a geocode service resource includes fields with the following names: Street, City, State and Zone, then the address argument is of the form: { Street: "", City: "", State: "", Zone: "" } Locators published using ArcGIS 10 or later support a single line address field which can be specified using the following syntax: var address = {"Single Line Input":"380 New York St, Redlands, CA 92373"}; Services Directory can be used to find out the required and optional address fields. outFields Optional The list of fields included in the returned result set. This list is a comma delimited list of field names. If you specify the shape field in the list of return fields, it is ignored. For non-intersection addresses you can specify the candidate fields as defined in the geocode service. For intersection addresses you can specify the intersection candidate fields. searchExtent Optional Defines the extent within which the geocode server will search. Requires ArcGIS Server version 10.1 or greater. </param>
...
esri.tasks.OffsetParameters = function () {
    ...
    /// <field name="offsetHow" type="String">Set to one of the following options: esriGeometryOffsetBevelledSquares off the corner after a given ratio distance. esriGeometryOffsetMiteredAttempts to allow extended offsets to naturally intersect. if the intersection occurs too far from a corner, the corner will be beveled off at a fixed distance. esriGeometryOffsetRoundedRounds the corner between extended offsets.</field>
...
esri.tasks.ParameterValue = function () {
    ...
    /// <field name="value" type="Object">The value of the parameter. The data structure of this value depends on the dataType. Data Type JavaScript Type Example GPString String "MyString" GPDouble Number (float) 12.56 GPLong Number (int) 345 GPBoolean Boolean true GPDate Date GPLinearUnit <LinearUnit> GPDataFile <DataFile> GPRasterData <RasterData> GPRecordSet <FeatureSet> GPRasterDataLayer <RasterData> GPFeatureRecordSetLayer <FeatureSet> GPMultiValue String[] ["Parcels", "Street Lights"] </field>
};
...
esri.tasks.PrintTemplate = function () {
    ...
    /// <field name="exportOptions" type="Object">Define the map width, height and dpi. Required when the layoutTemplate is set to 'MAP_ONLY'. exportOptions is an object with the following structure: { width, height, dpi }</field>
    ...
    /// <field name="layout" type="String">The layout used for the print output. The print service provides the following out-of-the-box templates:MAP_ONLYA3 LandscapeA3 PortraitA4 LandscapeA4 PortraitLetter ANSI A LandscapeLetter ANSI A PortraitTabloid ANSI B LandscapeTabloid ANSI B PortraitThe server administrator can add additional templates to the print service.</field>
    /// <field name="layoutOptions" type="Object">Define the layout elements using an object with the following properties: titleText The text used for the map title if the specified layout contains a title text element. authorText The text used for the author if the specified layout contains an author text element. copyrightText The text used for the copyright if the specified layout contains an copyright text element. scalebarUnit The units used for the scalebar. Valid values are 'Miles' | 'Kilometers' | 'Meters' | 'Feet' legendLayers An array of LegendLayer containing the id's of the layers that will be included in the legend. If legendLayers is not specified, all operational layers will be present in the legend. To specify that no layers will be included in the legend set legendLayer to an empty array. customTextElements Updated the text for a TextElement, that is not DynamicText, on the page layout. Values must be strings.</field>
    /// <field name="preserveScale" type="Boolean">Define whether the printed map will preserve the map scale or the map extent. Only one of these properties can be preserved to produce a high-resolution map for printing. If preserveScale is set to true, the map displayed in the print window will match the current scale of the map in the application. If preserveScale is false, the current extent of the map in the application displays in the printed map but the scale may change. The default value is true.</field>
...
esri.tasks.Query = function () {
    ...
    /// <field name="groupByFieldsForStatistics" type="String[]">One or more field names that will be used to group the statistics. groupByFieldsForStatistics is only valid when outStatistics have been defined. Requires ArcGIS Server service version 10.1 or greater</field>
    ...
    /// <field name="orderByFields" type="String[]">One or more field names that will be used to order the query results. Specfiy ASC (ascending) or DESC (descending) after the field name to control the order. The default order is ASC. orderByFields is only supported on dynamic layers and tables where supportsAdvancedQueries is true.Requires ArcGIS Server service version 10.1 or greater</field>
    ...
    /// <field name="outStatistics" type="StatisticDefinition[]">The definitions for one or more field-based statistic to be calculated. outStatistics is only supported on layers/tables where supportsStatistics is true. If outStatistics is specified the only other query parameters that will be used are groupByFieldsForStatistics, orderByFields, text, timeExtent and where.Requires ArcGIS Server service version 10.1 or greater</field>
    /// <field name="relationParam" type="String">The 'Shape Comparison Language' string to evaluate. Examples of valid strings are:RELATE(G1,G2,"FFFTTT***) dim (g1.boundary,g2.boundary) = lineardim (g1.exterior, g2.boundary)= linearThe string describes the spatial relationship to be tested when the spatial relationship is esriSpatialRelRelation The Shape Comparison Language EDN topic has additional details.</field>
...
esri.tasks.RelationParameters = function () {
    ...
    /// <field name="relationParam" type="String">The 'Shape Comparison Language' string to evaluate. Examples of valid strings are:RELATE(G1,G2,"FFFTTT***) dim (g1.boundary,g2.boundary) = lineardim (g1.exterior, g2.boundary)= linearThe string describes the spatial relationship to be tested when the relation parameter is specified as esr.tasks.RelationParamter.SPATIAL_REL_RELATION. The Shape Comparison Language EDN topic has additional details.</field>
};
...
esri.tasks.RouteParameters = function () {
    ...
    /// <field name="accumulateAttributes" type="String[]">The list of network attribute names to be accumulated with the analysis, i.e. which attributes should be returned as part of the response. The default is as defined in the specific routing network layer used in your RouteTask. You can specify any attributes names listed in the Service Directory under "Network Dataset -> Network Attributes" as "Usage Type: esriNAUTCost". See also Understanding the network attribute.</field>
    /// <field name="attributeParameterValues" type="Object[]">Each element in the array is an object that describes the parameter values. The object has the following properties:attributeName: The attribute name. String.parameterName: The parameter name. String.value: The parameter value. Number.</field>
    /// <field name="barriers" type="Object">The set of point barriers loaded as network locations during analysis. Can be either an instance of DataLayer or FeatureSet.At ArcGIS Server 10.1 an optional url property was added. Use this property to specify a REST query request to a Feature, Map or GP Service that returns a JSON feature set. The url property can be specified using DataFile Note that either the features or url property should be specified.</field>
    /// <field name="directionsLanguage" type="String">The language used when computing directions. The default is as defined in the specific routing network layer used in your RouteTask. By default, NAServer gets installed with en_US only - it is up to the server administrator to add additional languages.</field>
    /// <field name="directionsLengthUnits" type="String">The length units to use when computing directions. The default is as defined in the specific routing network layer used in your RouteTask.</field>
    /// <field name="directionsOutputType" type="String">Defines the amount of direction information returned. The default value is standard.</field>
    /// <field name="directionsStyleName" type="String">The style to be used when returning directions. The default will be as defined in the network layer. View the REST layer description for your network service to see a list of supported styles.</field>
    ...
    /// <field name="findBestSequence" type="Boolean">If true, optimizes the order of the stops in the route while taking into account preserveFirstStop and preserveLastStop, if they are set to true. The default is as defined in the specific routing network layer used in your RouteTask. For more information, see Finding the best stop sequence and Routing with time windows.</field>
    ...
    /// <field name="impedanceAttribute" type="String">The network attribute name to be used as the impedance attribute in analysis. The default is as defined in the specific routing network layer used in your RouteTask. You can specify any attributes names listed in the Service Directory under "Network Dataset -> Network Attributes" as "Usage Type: esriNAUTCost". You can also specify a value of "none" to indicate that no network attributes should be used for impedance. If you specify an empty array, it will default to the default of the service. For example, set impedanceAttribute="Time" for quickest route and impedanceAttribute="Length" for shortest drive, assuming the service has those two esriNAUTCost attributes. For more information, see Understanding the network attribute.</field>
    /// <field name="outSpatialReference" type="SpatialReference">The well-known ID of the spatial reference for the geometries returned with the analysis results. If not specified, the geometries are returned in the spatial reference of the map. See Projected Coordinate Systems and Geographic Coordinate Systems for the list of supported spatial references.</field>
    /// <field name="outputGeometryPrecision" type="Number">The precision of the output geometry after generalization. If 0, no generalization of output geometry is performed. If present and positive, it represents the MaximumAllowableOffset parameter - generalization is performed according to IPolycurve.Generalize.</field>
    /// <field name="outputGeometryPrecisionUnits" type="String">The units of the output geometry precision. The default value is "esriUnknownUnits".</field>
    ...
    /// <field name="preserveFirstStop" type="Boolean">If true, keeps the first stop fixed in the sequence even when findBestSequence is true. Only applicable if findBestSequence is true. The default is as defined in the specific routing network layer used in your RouteTask.</field>
    /// <field name="preserveLastStop" type="Boolean">If true, keeps the last stop fixed in the sequence even when findBestSequence is true. Only applicable if findBestSequence is true. The default is as defined in the specific routing network layer used in your RouteTask.</field>
    /// <field name="restrictUTurns" type="String">Specifies how U-Turns should be handled. The default is as defined in the specific routing network layer used in your RouteTask. See NAUTurn for a list of valid values.</field>
    /// <field name="restrictionAttributes" type="String[]">The list of network attribute names to be used as restrictions with the analysis. The default is as defined in the specific routing network layer used in your RouteTask. Possible values are listed in the Service Directory under "Network Dataset -> Network Attributes". You can also specify a value of "none" to indicate that no network attributes should be used as restrictions. If you specify an empty array, it will default to the default of the service.</field>
    ...
    /// <field name="stops" type="Object">The set of stops loaded as network locations during analysis. Can be either an instance of DataLayer or FeatureSet. When RouteParameters.stops takes a FeatureSet, each feature in the FeatureSet must have a defined spatial reference. If the feature contains x and y attributes, those values are used for the stop, even if the feature includes geometry. For the list of attributes that can be set on a stop, see the "Stop properties" section in Finding the best route.At ArcGIS Server 10.1 an optional url property was added. Use this property to specify a REST query request to a Feature, Map or GP Service that returns a JSON feature set. The url property can be specified using DataFile Note that either the features or url property should be specified. </field>
    /// <field name="useHierarchy" type="Boolean">If true, the hierarchy attribute for the network should be used in analysis. The default is as defined in the specific routing network layer used in your RouteTask.</field>
    /// <field name="useTimeWindows" type="Boolean">If true, time windows should be used in the analysis. The default is as defined in the specific routing network layer used in your RouteTask.</field>
};
...
esri.tasks.ServiceAreaParameters = function () {
    ...
    /// <field name="impedanceAttribute" type="String">The network attribute name used as the impedance attribute in analysis. The default is as defined in the specific routing network layer used in your RouteTask. You can specify any attributes names listed in the Service Directory under Network Dataset > Network Attributes as Usage Type: esriNAUTCost. You can also specify a value of none to indicate that no network attributes should be used for impedance. If you specify an empty string, it will use the default of the service.For example, set impedanceAttribute=Time for quickest route and impedanceAttribute=Length for shortest drive. Assuming the service has those two esriNAUTCost attributes.View the Understanding the network attribute ArcGIS desktop help topic for more details.</field>
    ...
    /// <field name="outputGeometryPrecisionUnits" type="String">The units of the output geometry precision. The default value is esriUnknownUnits.</field>
    /// <field name="outputLines" type="String">The type of output lines to be generated in the result. The default is as defined in the specific routing network layer used in your RouteTask. See NAOutputLine for a list of valid values.</field>
    /// <field name="outputPolygons" type="String">The type of output polygons to be generated in the result. The default is as defined in the specific routing network layer used in your ServiceAreaTask. See NAOutputPolygon for a list of valid values.</field>
    ...
    /// <field name="timeOfDay" type="Date">Local date and time at the facility. if travelDirection set to TO_FACILITY, timeOfDay value specifies arrival time at the facility. if travelDirection set to FROM_FACILITY, timeOfDay specifies departure time from the facility. Requires ArcGIS Server service version 10.1 or greater.</field>
    /// <field name="travelDirection" type="String">Options for traveling to or from the facility. Default values are defined by the newtork layer. See NATravelDirection for a list of valid values.</field>
    ...
    /// <field name="useHierarchy" type="Boolean">When true, the hierarchy attributes for the network will be used in analysis. The default value is defined in the network layer. useHierarchy cannot be used in conjunction with outputLines. Requires ArcGIS Server service version 10.1 or greater.</field>
};
...
esri.tasks.TrimExtendParameters = function () {
    ...
    /// <field name="extendHow" type="String">A flag used along with the trimExtend operation. DEFAULT_CURVE_EXTENSION Default value. The extension considers both ends of the path. The old ends remain and new points are added to the extended ends. The new points have attributes that are extrapolated from existing adjacent segments. RELOCATE_ENDSWhen an extension is performed at an end, relocate the end point to the new position. KEEP_END_ATTRIBUTESWhen an extension is performed at an end, do not extrapolate the end segments attributes for the new point. Instead, the attributes will be the same as the current end. NO_END_ATTRIBUTESWhen an extension is performed at an end, do not extrapolate the end segments attributes for the new point. Instead the attributes will be empty. NO_EXTEND_AT_FROM Do not extend the 'from' end of any path. NO_EXTEND_AT_TODo not extend the 'to' end of any path.</field>
...
esri.toolbars.Draw.prototype = 
{
    activate: function (geometryType,options) {
        ...
        /// <param name="options" type="Object" optional="true">Options that define the functionality of the draw toolbar. drawTime Determines how much time to wait before adding a new point when using a freehand tool. The default value is 75. showTooltips If true, tooltips are displayed when creating new graphics with the draw toolbar. The default value is true. tolerance Determines how far the mouse moves before adding a new point when using one of the freehand tools. The default value is 8. tooltipOffset Determines how farm to offset the tool tip from the mouse pointer. The default value is 15. </param>
    },
    deactivate: function () {
...
esri.toolbars.Edit.prototype = 
{
    ...
    getCurrentState: function () {
        /// <summary>An object with the following properties that describe the current state. Value Description tool Indicates the current tool. Valid values are listed in the constants table. <Graphic> graphic The graphic that is currently being edited. isModified Indicates if the graphic has been modified.</summary>
        /// <return type="Object">Object</return>
    },
    refresh: function () {
...
esri.virtualearth = function (map) {
    /// <summary>Creates a new Navigation object. A Map is a required parameter.</summary>
    /// <param name="map" type="Map" optional="false">Map the toolbar is associated with.</param>
};
